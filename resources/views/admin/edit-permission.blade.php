            ...
            <div class="wrapper d-flex flex-column flex-row-fluid" id="kt_wrapper">
                ...
                <div class="container">
                    ...
                    <div class="col-md-5 mx-2 ">
                        ...
                        @endif
                        <form action="{{ route('update', $editPermission->id) }}" method="POST">
                            @csrf
                            <div class="row">
                                <input type="text" name="name" value="{{ $editPermission->name }}" class="form-control my-2"
                                    placeholder="Add Permission">
                                @if ($errors->has('addPermission'))
                                    <span class="text-danger">{{ $errors->first('addPermission') }}</span>
                                @endif
                                <button type="submit" class="btn btn-info my-2">update Permission</button>
                            </div>
                        </form>
                    </div> 
                </div>
